...
# ── ALU opcodes from riscv_defs.v ─────────────────────────────
ALU_NONE         = 0x0
ALU_SHIFTL       = 0x1   # 4'b0001
ALU_SHIFTR       = 0x2   # 4'b0010
ALU_SHIFTR_ARITH = 0x3   # 4'b0011
ALU_ADD          = 0x4   # 4'b0100
# 0x5 unused
ALU_SUB          = 0x6   # 4'b0110
ALU_AND          = 0x7   # 4'b0111
ALU_OR           = 0x8   # 4'b1000
ALU_XOR          = 0x9   # 4'b1001
ALU_LESS_THAN    = 0xA   # 4'b1010
ALU_LESS_THAN_S  = 0xB   # 4'b1011
...
async def load_a(dut, value, op):
    ...
    ui_in[7]=1        → A load mode
    ui_in[6:5]=i      → byte index (0-3)
    ui_in[4:1]=op     → ALU op (latched on byte 0)
    uio_in[7:0]=byte  → data
    """
    for i in range(4):
        b = (value >> (8 * i)) & 0xFF
...
async def load_b(dut, value):
    ...
    ui_in[7]=0        → B load mode
    ui_in[6:5]=i      → byte index (0-3)
    ui_in[0]=0        → not read mode
    uio_in[7:0]=byte  → data
    """
    for i in range(4):
        b = (value >> (8 * i)) & 0xFF
        dut.ui_in.value  = (i << 5) & 0x60   # bit7=0, bit0=0
        dut.uio_in.value = b
        await ClockCycles(dut.clk, 1)
    # Extra settle cycle
    await ClockCycles(dut.clk, 1)
...
async def read_result(dut):
    ...
    ui_in[0]=1          → read mode
    uio_in[7:6]=i       → result byte select
    uo_out              → selected byte
    """
    result = 0
    for i in range(4):
        dut.ui_in.value  = 0x01             # read mode
        dut.uio_in.value = (i << 6)         # byte select on [7:6]
        await ClockCycles(dut.clk, 1)       # latch out_sel
        await ClockCycles(dut.clk, 1)       # read stable output
...
async def test_project(dut):
    ...
    await load_a(dut, 0xFF, ALU_XOR)
    await load_b(dut, 0xFF)
    result = await read_result(dut)
    ...
    await load_a(dut, 3, ALU_LESS_THAN)
    await load_b(dut, 5)
    result = await read_result(dut)
    ...
    await load_a(dut, 5, ALU_LESS_THAN)
    await load_b(dut, 3)
    result = await read_result(dut)
    dut._log.info(f"SLT result: {result}")
    ...
    # ── Test 9: SRL 16 >> 4 = 1 ───────────────────────────────
    dut._log.info("Test SRL: 16 >> 4 = 1")
    await load_a(dut, 16, ALU_SHIFTR)
    await load_b(dut, 4)
    result = await read_result(dut)
    dut._log.info(f"SRL result: {result}")
    assert result == 1, f"SRL failed: expected 1 got {result}"

    # ── Test 10: ADD 32-bit 0x12345678 + 0xFF = 0x12345777 ────
    dut._log.info("Test ADD 32-bit: 0x12345678 + 0xFF")
    await load_a(dut, 0x12345678, ALU_ADD)
    await load_b(dut, 0xFF)
    result = await read_result(dut)
    ...
    # ── Test 11: SUB with borrow 0x100 - 0x1 = 0xFF ──────────
    dut._log.info("Test SUB: 0x100 - 0x1 = 0xFF")
    await load_a(dut, 0x100, ALU_SUB)
    await load_b(dut, 0x1)
    result = await read_result(dut)
    dut._log.info(f"SUB borrow result: {hex(result)}")
    assert result == 0xFF, f"SUB borrow failed: expected 0xFF got {hex(result)}"
